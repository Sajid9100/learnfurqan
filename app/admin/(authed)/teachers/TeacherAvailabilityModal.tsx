...
export function TeacherAvailabilityModal({ teacher, onClose }: Props) {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch(`/api/admin/teachers/${teacher.id}/availability`);
  ...
  async function remove(kind: "rule" | "exception", rowId: string) {
    ...
    try {
      const r = await fetch(
        `/api/admin/teachers/${teacher.id}/availability?kind=${kind}&row_id=${rowId}`,
        { method: "DELETE" }
      );
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Failed");
        ...
        <div className="space-y-8">
          <section>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Each rule generates {teacher.class_duration_minutes ?? 30}-minute
              slots between start and end in the chosen timezone.
            </p>

            <div className="mt-3 space-y-2">
              {rules.length === 0 && (
                <p className="rounded-lg border border-dashed border-border bg-muted/20 px-3 py-3 text-sm text-muted-foreground">
                  No rules yet. This teacher will appear unavailable.
                </p>
              )}
              {rules.map((r) => (
                <div
          ...
          <section>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              ...
              etc.). <strong>Extra</strong>: add a one-off window.
            </p>
